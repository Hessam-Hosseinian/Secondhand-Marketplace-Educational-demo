...
public class ImageController {
 ...
 public ImageController(@Value("${app.upload.dir:uploads}") String value)throws IOException{directory=Paths.get(value).toAbsolutePath().normalize();Files.createDirectories(directory);}
 ...
 public ResponseEntity<Map<String,String>> upload(@RequestPart("file") MultipartFile file)throws IOException{
  ...
  if(file.getSize()>8*1024*1024)throw ApiException.bad("Image must be smaller than 8 MB");
  if(!ALLOWED.contains(file.getContentType()))throw ApiException.bad("Supported formats: JPG, PNG, WebP and GIF");
  ...
  return ResponseEntity.status(201).body(Map.of("imageUrl","http://localhost:8080/uploads/"+name));
 }
}
